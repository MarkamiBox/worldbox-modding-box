...
> Visual Studio나 컴파일러 같은 복잡한 도구는 **전혀 필요하지 않습니다**. NML이 모드 폴더 안의 `.cs` 텍스트 파일을 읽어 필요할 때 자동으로 컴파일해 줍니다. **메모장만으로도 첫 번째 모드를 완벽하게 만들 수 있습니다** :PES_OkHand:. 더 고급 도구는 나중에 기능이 아쉬워질 때 도입해도 늦지 않습니다.
...
> [!NOTE] 오래된 NCMS 모드 읽기
> NML은 예전 `[ModEntry]` 진입점 지원을 포함한 NCMS 호환 레이어를 내장하고 있습니다. 다만 그렇다고 게임 API 변경에 따른 호출까지 고쳐주지는 않습니다. 오래된 모드에 의존하기 전에 현재 게임 및 NML 버전으로 직접 테스트하세요. 이 가이드처럼 HelloBox는 `BasicMod<Main>`으로 시작하세요.

다음: **[모드 구조](#/nml/mod-structure)**.
