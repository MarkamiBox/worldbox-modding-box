...
```
> [!WARNING] 그림자는 직접 불러오세요, 안 그러면 게임이 모든 액터에 대해 불평합니다
> `ActorAssetLibrary`는 시작할 때 목록을 돌면서 각 액터에 `loadShadow()`를 호출하는데, 이 메서드는 `shadows/<shadow_texture>`의 스프라이트를 읽고 크기를 잽니다. 이는 여러분의 모드가 무언가를 등록하기 전에 일어났으므로, 여러분 액터의 그림자는 `(0.00, 0.00)`으로 남고 게임은 성체, 알, 새끼마다 한 번씩, 총 세 번 에셋 오류를 기록합니다 :wbfacepalm:.
>
> `loadShadow()`는 `internal`이므로, 가이드의 나머지 부분처럼 **퍼블리사이즈된** `Assembly-CSharp.dll`이 필요합니다. 없다면 대신 `asset.shadow = false;`를 설정하세요. 그림자는 없지만 오류도 없습니다.

> [!WARNING] `clone()`은 이미 등록합니다
> `AssetManager.<library>.clone(newId, sourceId)`는 내부에서 `add()`를 호출합니다. 모든 라이브러리가 이렇게 동작합니다. 그 뒤에 직접 `add()`를 호출하면 중복 등록입니다: 라이브러리는 첫 번째 복사본을 제거하고, 오류를 기록하고, 다시 추가합니다. 해는 없지만 진짜 오류를 찾기 어렵게 만드는 로그 소음이고, 검토자가 가장 먼저 발견할 부분입니다.
>
> 뒤집어 보면 좋은 소식입니다: **클론한 뒤에는 `base_stats`가 이미 존재하므로**, **[커스텀 특성](#/nml/custom-traits)**의 "스탯은 add 다음에" 규칙은 이미 지켜진 셈입니다.
...
첫날에 중요한 건 세 개뿐입니다: `civ`, `actor_size`, `name_locale`. 나머지는 생명체가 보이고 걸어 다닐 때까지 미뤄도 됩니다.
