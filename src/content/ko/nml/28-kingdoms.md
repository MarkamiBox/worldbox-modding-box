...
이 설정이 없으면 당신의 크리처는 클론 원본이 사용하던 왕국(대체로 인간)에 스폰되어 기괴한 상황을 연출하게 됩니다 :PES5_Hmmmm:.
...
왕국 에셋 하나만으로는 단순한 이름표에 불과합니다. 아주 공식적인 이름표지만, 어쨌든 이름표입니다. 온전한 세력을 이루기 위해 맞물려 돌아가는 라이브러리들은 다음과 같습니다:
...
정치적 상호작용을 제어하는 세 가지 소형 라이브러리가 있으며, 모두 계산 함수들의 목록으로 이루어져 있습니다. 감정은 없고, 숫자만 있습니다:
...
경로는 깃발이 그려질 때마다 개별 로드되므로 새로고침할 필요가 없습니다. 목록 길이를 초과하는 인덱스는 0으로 안전하게 대체되므로 모드로 생성된 세이브 파일도 모드 없이 정상적으로 열립니다. 바니라 파츠 크기에 맞추기 위해 **[UnityExplorer](#/toolbox/unity-explorer)**에서 확인해 보세요. 크기를 대충 짐작하면 그 깃발을 단 도시보다 더 큰 깃발이 나옵니다 :wbfacepalm:.
