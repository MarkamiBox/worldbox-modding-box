...
오직 `mod.json` 만 필수입니다. 다른 폴더들은 실제로 필요할 때 만들면 됩니다. `mod.json` 과 `Code/` 만 있어도 완벽하게 동작하는 진짜 모드입니다.

#### 각 폴더의 역할

- **`mod.json`**: 모드의 신분증입니다. 이 파일이 없으면 NML은 모드가 존재하는지도 모른 채 지나칩니다.
- **`icon.png`**: 게임 내 모드 목록 메뉴에서 표시될 미리보기 이미지입니다.
- **`Code/`**: 모든 `.cs` 소스 코드 파일(예: `Main.cs`)이 들어가는 폴더입니다. **게임이 실행될 때마다 NML이 자동으로 컴파일해 주므로**, 사용자가 직접 `.dll` 을 빌드할 필요도 없고 Visual Studio도 필요 없습니다.
- **`Locales/`**: 번역 파일(예: `en.json`)이 위치하는 곳입니다. 이 폴더가 없으면 추가한 아이템과 특성이 게임 내에서 원시 키 문자열 그대로 노출됩니다.
- **`GameResources/`**: 커스텀 텍스처, 픽셀 아트, 특성 아이콘, 무기 스프라이트, 효과음이 들어갑니다. NML이 이 이름으로 검색하므로 폴더명을 정확히 일치시켜야 합니다. 자세한 내용은 **[스프라이트 및 리소스](#/nml/sprites-and-resources)** 를 참고하세요.
...
- **`GUID`**: 모드의 고유 식별자이며, 관례상 모두 소문자로 `com.yourname.modname` 형식을 사용합니다. 모드의 주민등록번호와 같아서 다른 사람의 모드와 충돌하지 않도록 보장합니다. **한 번 정하면 절대 바꾸지 마세요**: 플레이어의 설정 파일 이름이 이 GUID를 기반으로 생성됩니다.
...
- **`: BasicMod<Main>`**: 모드의 공인 신분증입니다. NML에게 *"나는 정식 모드입니다"* 라고 전달하며, 그 대가로 NML이 로그 출력, 설정 창, 번역 기능을 무료로 제공합니다. `<Main>` 부분은 자신의 클래스 이름을 그대로 반복하는 것입니다. 다소 특이해 보이지만 항상 이렇게 작성합니다.
- **`protected override void OnModLoad()`**: 가장 중요한 메서드입니다. WorldBox가 부팅될 때 NML이 이 문을 한 번 두드립니다. 모드가 등록하는 모든 내용(특성, 아이템, 권능)은 이 `{ }` 안에 들어갑니다.
...
> [!TIP] 기존의 고전적인 방식
> 이전 세대의 모드에서는 이런 식으로 작성된 것을 볼 수 있습니다:
> ```csharp
