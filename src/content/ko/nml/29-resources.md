...
자원이란 도시가 보관하고, 거래하고, 먹거나, 대장간에서 주조하는 모든 대상을 뜻합니다. 밀, 빵, 돌, 미스릴, 뼈, 보석 등이 여기에 해당합니다. 이들은 `AssetManager.resources`에 위치하며, 전체 경제 시스템의 근간을 이룹니다. 농장이 재배하는 것, 제빵사가 굽는 것, 대장장이가 필요로 하는 것, 배고픈 시민이 먹는 것이 모두 자원입니다.
...
자원은 두 개의 그래픽을 필요로 하며, 이들은 **완전히 다른 방식**으로 경로를 찾습니다. 많은 모더들이 여기서 실수를 범합니다:
