...
가이드를 다 마칠 때쯤이면 HelloBox에는 약 20여 개의 파일이 포함되며, 여러분은 그 모든 코드를 직접 작성하게 됩니다: 전용 탭이 있는 유닛 특성과 문화 특성, 무기와 전용 인챈트, 상태 효과, 전리품 드롭, 구름, 타일, 요리 레시피, 투사체, 세계 법률, 전용 버튼이 달린 신의 권능, 창, 설정 패널, 건물, 세력, 생명체, 재앙, 커스텀 AI, 그리고 게임의 고정 규칙을 비트는 Harmony 패치까지.
...
이 창은 나중에 모드를 **끄고 싶을 때** 사용하는 창이기도 합니다. 아이콘을 클릭하여 토글할 수 있으며, 대부분의 모드는 게임을 재시작해야 변경 사항이 완전히 적용됩니다.
