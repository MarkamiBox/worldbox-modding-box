...
이후에 이어지는 문서들을 제대로 이해하려면 먼저 이 문서를 완벽히 파악해야 합니다. WorldBox에 존재하는 모든 요소(특성 (trait), 무기, 건물 (building), 타일, 구름 (cloud), 왕국 (kingdom))는 **라이브러리**에 저장된 **에셋**이며, 게임 내의 모든 라이브러리는 동일한 4개의 메서드를 공유하는 똑같은 클래스 구조를 가집니다.
