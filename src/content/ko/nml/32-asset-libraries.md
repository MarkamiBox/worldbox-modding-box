...
게임은 부팅 시 129개 라이브러리를 먼저 모두 빌드하고, 그 위에서 `post_init()`을 실행한 후, **그다음에** NML이 여러분의 모드를 로드합니다. 여기서 파생되는 두 가지 중요한 결론, 저도 포함해서요:

- **라이브러리가 `post_init`에서 자동으로 처리하는 로직은 이미 지나갔습니다.** 예를 들어 액터 특성의 기본 `path_icon` 자동 부여 작업은 이미 끝났습니다. 여러분의 특성은 그 시점에 존재하지 않았으므로 자동 혜택을 받지 못합니다. 직접 아이콘을 지정하세요.
- **여러분의 `OnModLoad`가 실행될 때 모든 바닐라 에셋은 이미 완벽히 존재합니다.** 따라서 `get("human")`도, `clone(..., "human")`도, 바닐라 에셋의 직접 수정도 모두 정상 작동합니다. 너무 일찍 실행될 걱정은 하지 않아도 됩니다.

> [!NOTE] 이 메서드들을 패치해도 바닐라 콘텐츠는 바뀌지 않습니다
> `has`, `get`, `add`, `clone`, `post_init`은 NML이 모드를 단 하나라도 로드하기 전, 게임 부팅 시점에 129개 라이브러리 전체에 대해 실행됩니다. 이들 중 어느 것에 Harmony 패치를 걸어도 영향을 주는 건 모드가 로드된 *이후*에 발생하는 호출뿐이며, 그 시점에 이미 끝나버린 바닐라 등록에는 전혀 영향을 주지 못합니다. 바닐라 콘텐츠를 바꾸고 싶다면, 이 페이지의 다른 부분과 마찬가지로 이후에 `get()`으로 변경하세요.
