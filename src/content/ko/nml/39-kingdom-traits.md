...
바닐라는 이 시스템을 단 한 가지 목적, 즉 세율 설정에만 활용합니다. 그 덕분에 7가지 특성 시스템 중 가장 규모가 작고 텅 빈 영역이며, 새로운 정책을 도입하기에 가장 흥미진진한 무대이기도 합니다. 누구도 이 공간을 두고 경쟁하지 않습니다.
...
`base_stats` 가 무의미하므로, 왕국 특성은 다음 두 가지 방법 중 하나로 제 몫을 다하게 됩니다.
