...
타일 타입에는 약 100여 개의 필드가 존재하며, 그중 대부분은 특정 바닐라 타일 하나만을 위해 존재합니다. 가장 가까운 원본을 클론하세요:
...
`main_type`과 `top_type`은 모두 `null`일 수 있습니다. 접근하기 전에 반드시 null 체크를 거치세요. 이는 맵을 탐색하는 모드에서 가장 흔하게 발생하는 크래시 원인입니다.
