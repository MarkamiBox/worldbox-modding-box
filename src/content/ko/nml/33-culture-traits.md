...
7가지 특성 시스템 중 문화는 가장 넓은 파급력을 지닙니다. 문화는 도시를 따라 전파되고, 창시자보다 오래 존속하며, 소속된 모든 단일 유닛에게 능력치를 결합합니다. 1시간의 플레이 동안 세계 전역으로 잔잔하게 번져 나가는 모드를 원한다면 이 라이브러리가 정답입니다. 큰 영향력에는 큰 책임이 따릅니다 :PES5_Menace:.
