...
코드 한 줄 바꾼 것을 테스트하기 위해 매번 WorldBox를 재시작하는 것은 모딩 시간의 대부분을 낭비하게 만듭니다. NML은 게임이 실행 중인 동안 모드를 다시 컴파일하고 지정된 메서드를 교체(핫픽스)할 수 있습니다.

1. 메인 클래스에 `IReloadable` 인터페이스를 구현합니다. 이는 `Reload()` 메서드 하나뿐입니다. HelloBox의 구현은 **[완성된 모드](#/nml/all-together)**에서 확인할 수 있습니다.
2. 리로드 버튼은 `Config.isEditor`가 `true`일 때만 표시됩니다. HelloBox는 기본값이 `false`인 `DevReload` 설정을 통해 이를 켭니다.
...
> [!WARNING] `Config.isEditor`는 게임 자체의 내부 스위치입니다
> WorldBox에 Unity 에디터 내에서 실행 중이라고 알리는 플래그이므로, 일부 UI가 모바일 레이아웃으로 변경되거나 시작 시 자체 파괴되는 객체들이 생길 수 있습니다. 개발 테스트 용도로만 켜고 배포 모드에서는 절대 켜지 마세요.
