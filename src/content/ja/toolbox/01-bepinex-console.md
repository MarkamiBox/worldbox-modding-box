...
Unityゲームが起動する直前に割り込んで動作するModローダーです。WorldBox のMod制作者がこれを使う目的は主に2つあります。リアルタイムコンソールの表示と、**UnityExplorer** の起動です（これについては専用の解説ページがあります）。ここにいるのは文明的なNML使いですが、このコンソールは使わないにはもったいなさすぎます :trollface:。Modが必要とした際にNMLが自動で導入してくれることもありますが、自分で導入しておけば設定を完全に掌握できます。
