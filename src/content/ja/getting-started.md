...
> Visual Studio や専門のコンパイラを用意する必要は**一切ありません**。NML が Mod フォルダ内の `.cs` テキストファイルを読み取り、必要なタイミングで自動的にコンパイルしてくれます。**最初の Mod を書くなら Windows 標準の「メモ帳」で十分です** :PES_OkHand:。不便を感じてからより良いツールに乗り換えれば問題ありません。
...
> [!NOTE] Steam ワークショップ版との併用は不要です
> NML はゲーム起動のたびに自動で新バージョンを確認し、自身を置き換えます。つまり上記の手動インストールだけで常に最新の状態が保たれます。[Steam ワークショップの NML ページ](https://steamcommunity.com/sharedfiles/filedetails/?id=3080294469) も存在しますが、手動でインストールした直後にこちらも購読すると、二つのコピーが競合し「Mod が表示されない」「Mod が消えない」といった不具合の典型的な原因になります。どちらか一方だけにしてください。なお、ワークショップで何かを購読した場合、Mod 一覧でチェックを外すことは購読解除とは別の操作です。

## 4. テキストエディタの準備

プレーンテキストを保存できるものなら何でも使えます。「あると便利」順にざっと並べると：
...
- **[BepInEx Mod開発](#/toolbox/bepinex-modding)**：NMLのコンテンツではなく、Unityエンジンへの低レベルなフックが欲しい場合に、コンパイル済みの `.dll` プラグインを作ります。

> [!NOTE] 古い NCMS Mod を読み込む場合
> NML には NCMS 互換レイヤーが含まれており、旧来のエントリーポイントである `[ModEntry]` にも対応しています。ただし、変更されたゲーム API への呼び出しまでは修復してくれません。古い Mod を使う前には、手元のゲームバージョンと NML バージョンで実際に動作を確認してください。本ガイドのように、HelloBox は `BasicMod<Main>` で始めましょう。

次へ: **[Modの基本構造](#/nml/mod-structure)**.
