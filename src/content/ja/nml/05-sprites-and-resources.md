...
| `RectX` / `RectY` | スプライト矩形のオフセットです。どちらも既定値は `0` で、NMLのドキュメントでも `0` のままにしておくよう案内されています |

`Default` は、`Specific` で指定されていない同じフォルダ内の全ファイルに適用されます。

> [!NOTE] オフセットはアトラス用のレシピではない
> ドキュメント化されている `sprites.json` の設定には、矩形の幅や高さのフィールドがありません。1枚のシートを切り出すためにそうしたフィールドを勝手に作らないでください。アトラスやスプライトシートについては、NMLはUnityの `.meta` ファイルを使う方法をドキュメント化しています。
...
次へ: **[Mod設定](#/nml/mod-config)** または **[カスタムウィンドウ](#/nml/custom-windows)**。
