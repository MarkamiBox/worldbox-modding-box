...
```
> [!WARNING] 影は自分で読み込む。さもないと全アクターについて文句を言われる
> `ActorAssetLibrary` は起動時にリストを巡回し、各アクターで `loadShadow()` を呼んで `shadows/<shadow_texture>` のスプライトを読み込み、サイズを測ります。これはあなたのModが何かを登録する前に済んでいるので、あなたのアクターの影は `(0.00, 0.00)` のままとなり、ゲームは成体、卵、幼体の分で3回アセットエラーを記録します :wbfacepalm:。
>
> `loadShadow()` は `internal` なので、ガイドの他の部分と同じく **publicize済み** の `Assembly-CSharp.dll` が必要です。ない場合は代わりに `asset.shadow = false;` を設定してください。影はなくなりますが、エラーも出ません。

> [!WARNING] `clone()` はすでに登録している
> `AssetManager.<library>.clone(newId, sourceId)` は内部で `add()` を呼びます。どのライブラリも同じです。その後で自分で `add()` を呼ぶと二重登録になり、ライブラリは最初のコピーを消し、エラーを記録し、もう一度追加します。害はありませんが、本当のエラーを見つけにくくするログのノイズで、レビューする人が真っ先に気づく点です。
>
> 裏を返せば朗報です：**クローンの後では `base_stats` はすでに存在する**ので、**[カスタム特性](#/nml/custom-traits)** の「ステータスはaddの後」ルールはすでに満たされています。
