...
**宗教**（religion）は都市や王国（kingdom）に属し、布教によって広がり、書物を執筆し、**儀式**を執り行います。儀式とは信者たちが自発的に試みる、世界を変貌させる企てです。宗教特性（trait）とは、そうした信仰箇条の一つです。
...
> これは `base_stats` が `Actor` に一切反映されない唯一の特性システムです。`Actor.updateStats()` が合成するのは亜種（subspecies）・氏族（clan）・言語・文化（culture）のみであり、**宗教はそのリストに含まれていません。**
>
...
`plot_id` を持つ宗教特性は**儀式**になります。宗教はその儀式を `possible_rites` に蓄積し、陰謀（plot）の実行条件が揃うと指導者や司祭が自律的にそれを試みます。あなたは信仰を書き、残りは司祭がやってくれます :wbpray:。
...
また、宗教特性は呪文（spell）や決断を付与することもでき、定期タイマーよりもこちらの方が自然な仕上がりになることが多いです:
