...
# パワータブ & ボタン :wbfingerclick:

神の力を登録しました。しかし、誰もそれをクリックすることはできません。なぜなら `GodPower` は単なるデータにすぎないからです。プレイヤーが実際に押すのは **PowerButton** であり、ボタンは画面下部バーの **タブ** の中に存在します。
...
private static void Buttons()
{
    // 神の力：カーソルを武装させ、次のクリックで click_action を実行
    PowerButtonCreator.CreateGodPowerButton(STRIKE, Icon("iconHelloStrike"), tab.transform);
    // ... 各機能に1つずつ、上の完全なファイルを参照 ...

    // 普通のボタン：メソッドを実行するだけ、マップクリック不要
    PowerButtonCreator.CreateSimpleButton("hello_panel", HelloWindow.Toggle, Icon("iconHelloPanel"), tab.transform);
}
```
...
`null` スプライトを渡すと、領域だけを占有して何も描画されないボタンになってしまいます。プレイヤーもあなたもそこにボタンがあることに気づけず、バグ報告には「MODが何も動作しません」と書かれることになります :PES_Pathetic:。
