...
ガイドを終える頃には、HelloBoxには約40個のファイルが含まれ、そのすべてのコードを自分で書いたことになります：専用タブを持つユニット特徴と文化（culture）特徴、武器とそのエンチャント、ステータス効果（status）、ドロップ品、雲（cloud）、タイル、料理レシピ、発射物、世界の法則（world law）、専用ボタン付きの神の力、ウィンドウ、設定パネル、建物（building）、派閥、クリーチャー、災害（disaster）、専用AI、そしてゲームの既定ルールを上書きするHarmonyパッチまで。
...
WorldBoxを起動し、メインメニューから **Mods** ウィンドウを開きます。**HelloBox** がリストにあり、すでにオンになっているはずです。自分で `Mods/` に置いたModは、NMLが初めて見つけた時点で有効になります。

後でModを **オフ** にするのもこのウィンドウです。アイコンをクリックすると切り替わり、ほとんどのModは再起動するまでそれに気づきません :PES4_AlrightThen:。

> [!WARNING] Modsウィンドウが全くない？ 実験モードがオフです
> NMLは **設定 -> Experimental Mode** がオンのときだけModを読み込みます。そしてゲームは **WorldBoxのアップデートのたびに自動でそれをオフにします**：保存された `last_used_version` と今起動したバージョンを比べ、違っていればフラグを `false` に戻すのです。「昨日は動いていたのに何も変えていない」はほぼ確実にこれです。オンに戻して再起動してください。

> [!TIP] リストにすら出てこない？
> それならNMLは一度もそれを見ていません。10回中9回は `mod.json` ではなく `mod.json.txt` になっているか、フォルダーが `worldbox\Mods/` 以外の場所にあります。完全なリストは **[トラブルシューティング](#/troubleshooting)** にあります。
