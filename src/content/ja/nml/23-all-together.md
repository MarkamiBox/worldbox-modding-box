...
| 文化（culture）・宗教（religion）・亜種（subspecies）・クラン・言語・王国（kingdom）のtrait | それぞれの窓、システムごとに1つ |
| 武器、その付与効果、そして両方を入れるカテゴリ | ユニットの手、装備タブ |
| ステータス効果（status） | 生物の頭の上、専用アイコン付き |
| ドロップ、それを降らせる雲（cloud）、そして飛び道具 | マップ、空中、戦闘のまっただ中 |
| タイル | 地形、すべての下 |
| 食べ物のレシピ | 都市の備蓄 |
| ワールドロー | ワールドローの窓 |
| 神の力、そのタブとボタン | 画面下の神力バー |
| ウィンドウ | 自分が置くと決めた場所 |
| 建物（building） | 誰かが建てた瞬間の都市 |
| 王国と、そこに属する生物 | マップ、湧いて戦うところ |
| 災害（disaster） | 災害メニュー |
| 専用のAIジョブ | 目的を持ってどこかへ歩く生物 |
| 意思決定（decision）、都市の職業、手持ちツール | 松明を持って徘徊するウィスプ、都市ごとに1人の番人 |
| 戦闘アクション | 俊足特性（trait）を持つユニットが接近前に火の粉を投擲 |
| 遺伝子、性格、本の種類、旗パーツ | ゲノム、統治者、図書館、旗の紋章 |
| 友好度、忠誠度、幸福度イベント | 外交画面と都市の内訳一覧 |
| 陰謀（plot）・計画 | 指導者が火の粉の祭典を企てた際の陰謀リスト |
| ワールド時代（world age）とワールド挙動 | 時代の車輪とワールドタイマー |
| 実績（achievement） | ウィスプが10体になった際の実績ウィンドウ |
...
応用編のページには、他にも任意のレシピがあります: **[手動パッチ](#/nml/harmony-patches)**、**[タイマーとコルーチン](#/nml/update-loops)**、**[マップ生成](#/nml/map-generation)**、**[ゲームオプション](#/nml/game-options)**、**[他のModとの連携](#/nml/other-mods)**。これらは必要になったときに足すものです。このダウンロードには含まれていません。
...
3. **アイテムや建物より先に資源（resource）**: どちらも建設・製造コストとして資源を要求するためです。
4. **アイテムより先にモディファイア**: 武器は付与可能なモディファイアの一覧を保持するためです。
5. **アクターより先に国家（Kingdom）**: アクターは野生状態および文明状態での所属国家IDを指定するためです。
6. **ボタンより先にパワー（GodPower）**: `PowerButtonCreator` はIDでパワーを検索するため、存在しないパワーに結びつけられたボタンは機能しません。
