...
2. NMLのアクティブなModメニューでは、`IReloadable` を実装したModにリロードボタンが自動で表示されます。（古いModリストではボタンを出すのに `Config.isEditor = true` が必要でしたが、メインメニューではその手間はいりません。）
...
> [!NOTE] もし `Config.isEditor` を切り替えるなら
> `Config.isEditor` はゲーム内部のUnity用スイッチです。手動でオンにすると、WorldBoxはUnityエディター内で動いていると思い込み、一部のUIがモバイル用レイアウトに切り替わります。最新のNMLで `IReloadable` を使うなら不要なので、触らないでおきましょう。
