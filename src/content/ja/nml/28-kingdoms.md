...
これを忘れると、クローン元の生物が所属していた王国（大抵は人間）にスポーンしてしまい、奇妙な光景を生み出すことになります :PES5_Hmmmm:。
...
王国アセット単体は単なるラベルに過ぎません。とても公式なラベルですが、ラベルはラベルです。完全な勢力を成立させるために連携するライブラリは以下の通りです：
...
3つの小さなライブラリが政治の力学を制御しており、いずれも計算デリゲートのリストとして構成されています。感情は関係なく、数字だけです：
...
パスは旗が描画される際に個別に読み込まれるため、キャッシュ更新などの処理は不要です。リストの末尾を超えるインデックスは自動的に 0 にフォールバックするため、Mod 導入下で作成されたセーブデータも Mod なしで問題なく開くことができます。バニラのパーツサイズに合わせるため、描画前に **[UnityExplorer](#/toolbox/unity-explorer)** で確認しておきましょう。サイズを当てずっぽうにすると、掲げている都市より大きな旗が出来上がります :wbfacepalm:。
