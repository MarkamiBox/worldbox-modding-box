...
ゲームは起動時にまず129個の全ライブラリを構築し、それらに対して `post_init()` を実行し、**その後に** NMLがあなたのModをロードします。ここから生じる重要なポイントが2つあります。私も含めて：

- **ライブラリが `post_init` 内で自動的に行う処理は、すべて完了済みです。** 例えばアクター特性のデフォルトの `path_icon` の自動補完などは既に終わっています。あなたの自作特性はその時点では存在しなかったため、自動設定されません。自分で設定してください。
- **あなたの `OnModLoad` が走る時点で、すべてのバニラアセットが既に実体化されています。** そのため `get("human")` や `clone(..., "human")`、バニラコンテンツのインプレース書き換えはすべて問題なく機能します。「早すぎて読み込まれていない」ということはあり得ません。

> [!NOTE] これらのメソッドへのパッチはバニラコンテンツに影響しません
> `has`、`get`、`add`、`clone`、`post_init` はすべて、NMLが1つでもModをロードする前に、ゲーム起動時点で129個のライブラリに対して実行されます。これらにHarmonyパッチを当てても、影響するのは自分のModがロードされた*後*に行われる呼び出しだけで、その時点で既に完了しているバニラの登録処理には一切影響しません。バニラコンテンツを変更したい場合は、このページの他の部分と同様に、後から `get()` で変更してください。
