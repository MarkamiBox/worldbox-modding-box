...
陰謀には3つのローカライズキーが必要です（陰謀名、進行中の説明文、ベース説明文）。2つ目の進行中テキストでは `$initiator_actor$`, `$initiator_city$`, `$initiator_kingdom$`, `$target_kingdom$` などの変数が自動で置換されます。
...
> [!TIP] 強制実行によるテスト
> 指導者が自発的に陰謀を選んでくれるのを待つのは時間がかかります。ユニットを選択し、インスペクターウィンドウの陰謀タブから手動で強制実行（Force）してください。対象役職に就いている必要はありますが、資金消費なしですぐに開始できます。自作の `action` を手早くテストするのに最適です :PES2_EvilPlan:.
