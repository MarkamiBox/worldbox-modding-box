...
Tutto questo viene eseguito dal `click_action` di un potere divino (GodPower), da `Update()`, o da un world behaviour, e **mai** da `OnModLoad`, dove non esiste ancora alcun mondo. Vedi **[Log e debugging](#/nml/logs-and-debugging)** per la condizione di sicurezza, e **[Ogni frame](#/nml/update-loops)** per farlo da `Update()` senza costare il framerate al giocatore.
...
Un ciclo così va bene una volta, su un clic. Ogni frame su ogni edificio, no: eseguilo su un timer, vedi **[Ogni frame](#/nml/update-loops)**.
...
`getParents()` restituisce solo i genitori ancora in vita: cerca ciascun ID con `World.world.units.get(id)` e salta qualsiasi elemento mancante o morto. Gli ID rimangono nei dati dell'unità per sempre, ma il gioco non conserva alcuna memoria delle persone che vi stavano dietro. Un albero genealogico che ricordi i morti deve scrivere ciò che gli serve nei dati di ogni figlio alla nascita, vedi **[Ricordare le cose](#/nml/saving-data)**. Anche il mondo ha un proprio archivio, ma è un'unica lista piatta di chiavi, non un posto dove tenere diecimila alberi genealogici :PES_ThinkAboutIt:.
