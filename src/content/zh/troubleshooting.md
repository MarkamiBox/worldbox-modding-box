...
| 症状 | |
| --- | --- |
| 满屏红字，`Missing className: NeoModLoader (1).WorldBoxMod` | [跳转](#满屏红字-missing-classname) |
| NML 本来好好的，游戏一更新模组全变红或显示 "failed" | [跳转](#模组在游戏更新后变红或显示-failed) |
| 游戏处于旧版本，NML 无法加载 | [跳转](#游戏处于旧版本) |
| 开启模组后游戏卡顿或完全冻结 | [跳转](#开启模组后游戏卡顿或完全冻结) |
| 存档世界无法再加载 | [跳转](#存档世界无法再加载) |
| BepInEx 模组已放入但游戏里毫无反应 | [跳转](#bepinex-模组已放入但游戏里毫无反应) |
| 你已经删除了模组，但它依然在游戏中生效 | [跳转](#你已经删除了模组-但它依然在游戏中生效) |
| 游戏压根无法启动 | [跳转](#游戏压根无法启动) |
...
| 症状 | |
| --- | --- |
| 在设置属性（stats）的那一行崩溃 | [跳转](#在设置属性的那一行崩溃) |
| 同样的崩溃，顺序明明是对的 | [跳转](#同样的崩溃-顺序明明是对的) |
| 你的建筑（building）一出现就死，或者没有大小 | [跳转](#你的建筑一出现就死-或者没有大小) |
| 注册了，但哪个标签页里都没有 | [跳转](#注册了-但哪个标签页里都没有) |
| 显示的是 `trait_hello_swift` 而不是名字 | [跳转](#显示的是-trait-hello-swift-而不是名字) |
| 名字对特质（trait）有效，对物品（item）、状态（status）、神力（GodPower）无效 | [跳转](#名字对特质有效-对物品-状态-神力无效) |
| 图标是个空洞 | [跳转](#图标是个空洞) |
...
| 你的地块（tile）画得上去，然后地图渲染器报错 | [跳转](#你的地块画得上去-然后地图渲染器报错) |
...
| 添加世界行为（behaviour）后，每帧都抛出异常报错 | [跳转](#添加世界行为后-每帧都抛出异常报错) |
| 自然灾害在尝试写入世界日志时发生崩溃 | [跳转](#自然灾害在尝试写入世界日志时发生崩溃) |
| 未定义 action 的自然灾害在被抽中时发生崩溃 | [跳转](#未定义-action-的自然灾害在被抽中时发生崩溃) |
| 领袖在扫描并评估自制阴谋（plot）时触发崩溃 | [跳转](#领袖在扫描并评估自制阴谋时触发崩溃) |
| 自制决策（decision）、阴谋、基因或武器已成功注册，但在游戏里从未被触发或制造 | [跳转](#自制决策-阴谋-基因或武器已成功注册-但在游戏里从未被触发或制造) |
...
本节专门面向遇到模组故障的普通玩家，而非模组开发者。后续所有章节均假设你正在亲自编写模组代码。

### 满屏红字，missing className

- **你看到**：游戏画面上滚动刷出大量红字，提示 `previous errors repeated`、`YOU SHOULD RESTART THE GAME`，日志中显示 `Missing className: NeoModLoader (1).WorldBoxMod`。
- **原因**：文件没有被准确命名为 `NeoModLoader.dll`。浏览器在重复下载时会自动在文件名末尾附加 ` (1)`，而 NML 会读取自身的实际文件名。
- **解决**：关闭游戏，删除旧副本，将文件精确重命名为 `NeoModLoader.dll` 并重新启动。详细步骤参见 **[安装 NML](#/install-nml)**。

### 模组在游戏更新后变红或显示 failed

- **你看到**：在模组列表中，模组显示为红色、"failed"、`current failed, will load` 或 `<Mod> has been disabled due to an error`。在游戏更新前它一直能正常工作。
- **原因**：模组会调用游戏自身的代码。当 WorldBox 更新修改了这些底层代码时，针对旧版本编写的模组就会无法编译通过。这与 NML 本身无关，它只是如实报告了错误。
- **解决**：寻找该模组的更新版本（在 GameBanana 上按 **Updated** 排序）。如果没有新版本，请等待作者更新，或者回滚到旧版游戏，参见 **[常见问题](#/install-nml)**。切勿在 `Mods` 中同时保留同一个模组的两个版本“以防万一”：它们会发生严重冲突。

### 游戏处于旧版本

- **你看到**：NML 根本无法加载，或者日志中抛出 `MissingFieldException: Field not found: bool .Config.gameLoaded`。主菜单中的版本号比大家讨论的当前版本要旧。
- **原因**：游戏停留在 Steam 的 **测试版 (Beta)** 分支上（可能是很久以前为了参与早期测试而选择的），而你下载的 NML 则是面向当前正式版构建的。
- **解决**：Steam → 右键点击 WorldBox → **属性 → 测试版** → 选择 **无**。等待 Steam 完成更新，然后重新打开实验模式 :PES2_Shrug:。

### 开启模组后游戏卡顿或完全冻结

- **你看到**：帧率暴跌、画面频繁微卡，或者整个世界彻底停滞（但 UI 按钮依然可以点击）。关闭模组后游戏运行完全流畅。
- **原因**：几乎总是由某个在每个 tick 执行过重计算的模组引起的，通常是大型内容扩展类模组。两个试图修改同一系统的模组也可能会互相阻塞死锁。
- **解决**：禁用你的一半模组，重启游戏测试。如果问题依然存在，则问题出在开启的那一半中。继续对半二分排查，直到定位出具体哪个模组导致了卡顿。排查时只需在模组列表中禁用即可，无需彻底删除。阅读该模组的说明文档查看是否存在已知冲突，切勿同时运行同一个模组的完整版和“精简版”。

### 存档世界无法再加载

- **你看到**：加载存档时直接进入了一个空的新世界、卡在加载界面，或者在保存/读取时抛出 `NullReferenceException`。
- **原因**：该世界包含了来自某个现已被禁用、删除或损坏的模组中的单位、建筑或特质。游戏在遇到无法识别的自定义 ID 时无法正确反序列化。
- **解决**：重新启用该模组（或回滚到创建该存档时的游戏版本），进入世界并在游戏内手动清除所有由模组生成的内容，然后再安全禁用该模组。在测试大型内容模组前，务必先备份重要的世界存档 :PES_MonkaSweat:。

### BepInEx 模组已放入但游戏里毫无反应

- **你看到**：模组文件已经放进了 `BepInEx/plugins`，但游戏里完全没有任何效果，且 `BepInEx/config` 中也没有生成对应的配置文件。
- **原因**：要么是将下载的 zip 压缩包直接扔进了 `plugins` 导致没有解压，要么是 BepInEx 的管理器 GameObject 被游戏销毁了——这在部分电脑上需要专门的配置。
- **解决**：将 zip 解压后得到的**内部文件夹**放入 `BepInEx/plugins`，而不要放 zip 文件本身。接着打开 `BepInEx/config/BepInEx.cfg`，找到 `HideManagerGameObject = false`，将其修改为 `true`，保存并重启游戏。BepInEx 的配置指南参见 **[实时控制台](#/toolbox/bepinex-console)**。

### 你已经删除了模组，但它依然在游戏中生效

- **你看到**：`Mods` 文件夹里明明已经没有该模组了，但启动游戏时它依然会被加载运行。
- **原因**：你此前在 Steam 创意工坊中订阅了它，创意工坊的模组存放在 Steam 自身的专用目录中，而非你的 `Mods` 文件夹。
- **解决**：前往该模组的 Steam 创意工坊页面点击取消订阅。仅在游戏内的模组列表中取消勾选是不够的。

### 游戏压根无法启动

- **你看到**：即使移除了全部模组，WorldBox 依然在进入主菜单前崩溃闪退或卡死。
- **原因**：游戏本体的核心文件损坏或缺失，通常是由于此前不小心把文件复制到了错误的系统目录中。
- **解决**：Steam → 右键点击 WorldBox → **属性 → 已安装文件 → 验证游戏文件的完整性**。等待修复完成后，再逐个重新安装 NML 和模组。
...
- **解决**：资源（resource）管理器 → **查看 → 显示 → 文件扩展名**，然后确认真实文件名。用 VS Code 打开 `mod.json`，它会帮你标出 JSON 错误。
...
- **原因**：凡是能解锁的东西，`needs_to_be_explored` 默认都是 `true`：角色、七种特质、物品、物品词条和世界法则（world law）。
...
- **原因**：克隆草地块会保留 `is_biome = true`，但不会保留 `biome_asset`，后者是在启动时于 `linkAssets()` 里链接的。动物会把群系（biome）加进物种名里。
...
- **原因**：没有发射者的弹射物没有王国（kingdom），而管理器每帧都拿王国当字典的键。
...
- **原因**: 游戏在做随机抽取时读取的是启动期构建好的特定列表：`basic_plots`、AI 决策列表、基因突变池、武器生成池、时代（world age）轮盘格子池等。你的资产是在模组载入后追加的，不在这些预置列表内。
...
- **原因**：游戏把 `updateStats` 当作**并行**任务（task）运行：你的 Postfix 在工作线程上执行，同时处理很多单位。
