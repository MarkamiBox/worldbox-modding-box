...
你添加到游戏里的每一件东西（特质（trait）、物品（item）、能力、标签页、AI 任务（task）），在赋予本地化文本之前都会显示为像 `trait_hello_swift` 这样的原始键名。这是 Mod 开发中最枯燥的一章，而偷懒跳过它也是一个 Mod 看起来粗制滥造的第一大原因。（咳咳.. 我的 Mod 们.. 咳咳咳 :pensiveanimated: ）
...
| 神力（GodPower） | `<power_id>` | `<power_id>_description` |
| 神力标签页 | 你传入的 `locale_key` | 你传入的描述键 |
| 角色任务 | `task_unit_<task_id>` | - |
| 状态效果（status） | 你设置的 `locale_id` **字段** | 你设置的 `locale_description` **字段** |
| 世界法则（world law） | `<law_id>_title`（注意后缀） | `<law_id>_description` |
