...
这里是整套教程中最深奥的领域。本指南的其他页面都是在向游戏添加*物品与数据*，而本页添加的是**决策（decision）机制**：让一个生物完全自主、永久运行地决定下一步该做什么，并与世界上成千上万的其他生灵共同互动。别有压力 :PES_MonkaSweat:。
...
一个动作行为本质上是一个只包含一个核心方法的类。它接收当前生物作为参数，执行一步微小的逻辑，并返回一个 `BehResult` 状态（status）：
...
注意第二个行为：**复用原版节点**。游戏已经有走到某个地块（tile）、添加状态、寻找建筑（building）、攻击目标这些行为。自己写决策、借用现成的执行，这就是一个周末和一个月的区别。
...
> 每个生物在生成时都会初始化一个用于记录决策冷却 CD 的数组，大小按 2 的幂向上取整。原版共有 127 个决策，因此初始数组长度为 128：刚好仅剩 **1** 个空余槽位。若在模组加载前就已存在的生物被赋予了 2 个以上的新决策，其在思考时将抛出 `IndexOutOfRangeException`。模组加载后新生成的生物会按新总数正确扩容，因此 HelloBox 选择将决策直接绑定给自制生物，而非赋予原版生物特质（trait）。
