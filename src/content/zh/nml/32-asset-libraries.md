...
将 `sourceId` 上的每一个可序列化字段完整深拷贝到一个全新对象中，为其赋予 `newId`，**并自动在其内部调用 `add()` 完成注册**。方法返回克隆后的新实例。
...
> [!WARNING] 克隆（clone）之后切勿再次调用 add()
> 二次调用 `add()` 会导致刚生成的副本被剔除，在日志中留下一条刺眼的 `duplicate asset overwriting...`，然后重新加入。虽然代码通常也能跑通，但这会严重污染日志输出，掩盖真正的致命报错。

克隆是处理任何包含十个以上字段的事物（建筑、角色、物品、地块）的绝对首选姿态。你能直接继承一套经过原版严苛测试的成熟配置，只需聚焦于自己想要调整的那几个字段。
...
游戏在启动阶段会率先构建全部 129 个资源库，在它们身上触发 `post_init()`，**之后** NML 才会加载并执行你的模组代码。由此会产生两条经常让人迷茫的关键事实，我也不例外：

- **资源库在 `post_init` 中执行的自动化默认补全早已结束。** 比如角色特质会在那一阶段自动补全默认的 `path_icon`。而你的自定义特质因为当时还未出生，无法享受这项待遇。请在代码中自己手动指定。
- **当你的 `OnModLoad` 启动时，所有原版资源都早已准备就绪。** 所以 `get("human")` 可以放心调用，`clone(..., "human")` 绝不会报空，就地修改原版游戏内容也安全无虞。你永远不用担心自己的模组“启动太早”。
