...
到最后，HelloBox 会包含大约四十个代码文件，而且每一行代码都是你亲手写下的：拥有独立标签页的角色特质（trait）与文化（culture）特质、一把武器及其专属附魔（modifier）、一个状态效果（status）、掉落物、一片云朵、一种地图地块（tile）、一道食物配方、一个投掷物、一项世界法则（world law）、带专属按钮的神圣能力、一个弹窗、一个配置面板、一座建筑（building）、一个派系、一种全新生物、一场灾难（disaster）、定制的 AI 行为树，甚至还有一个用来打破游戏原有规则的 Harmony 补丁。
