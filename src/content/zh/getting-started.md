...
> [!NOTE] 此时此刻你甚至不需要懂编程
> 你**完全不需要**安装庞大的 Visual Studio、额外的编译链或任何类似工具。NML 会直接读取模组文件夹中的 `.cs` 源码文件，并在每次启动游戏时自动为你完成编译。**Windows 自带的“记事本”完全足够用来编写你的第一个模组** :PES_OkHand:。你可以等到以后确实感到功能受限时，再随时升级你的开发工具。
...
> [!TIP] 利用 Steam 创意工坊保持加载器更新
> 创意工坊中也提供了 [NML 创意工坊订阅项](https://steamcommunity.com/sharedfiles/filedetails/?id=3080294469)。单纯点击订阅并不能自动完成首次安装，但在你完成上述手动配置后，订阅它可以确保你的 NML 始终自动同步至最新版。
