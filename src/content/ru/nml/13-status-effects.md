...
Здесь есть ловушка, и все хоть раз в неё попадают :wbbre:. Поле `texture` — это **не** полный путь. Библиотека статусов автоматически добавляет префикс `effects/` перед загрузкой, поэтому нужно указывать только имя файла.
...
Если вы напишете путь с папкой, игра попытается найти `effects/effects/fx_hello_status`, ничего не найдёт и существо останется без визуального эффекта.

При этом поле `path_icon` у этого же ассета устроено иначе и *требует* полного пути — это маленькая иконка для интерфейса, а не накладываемый на карту спрайт.
