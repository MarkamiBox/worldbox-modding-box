...
Здесь есть важная деталь. Поле `texture` — это **не** полный путь. Библиотека статусов автоматически добавляет префикс `effects/` перед загрузкой, поэтому нужно указывать только имя файла.
