...
Первая строчка проигрывает звук в позиции конкретного тайла мира. HelloBox воспроизводит звук огненного шара при броске искры боевым действием, см. **[Снаряды, заклинания и эффекты](#/nml/projectiles-spells)**. Чтобы найти пути к звукам, поищите `event:/SFX/` в коде игры: там сотни записей, рассортированных по папкам.

> [!NOTE] Добавление новых звуков — отдельная тема
> FMOD-события зашиты в звуковые банки игры, и мод не может добавить их напрямую в FMOD. Воспроизведение собственных `.wav` файлов требует их ручной загрузки через Unity `AudioSource` в обход игровых настроек громкости. В этом руководстве это не рассматривается, потому что я никогда не модифицировал это и не собираюсь притворяться, что делал это.


### Добавление собственных звуков

NeoModLoader поддерживает нативные звуковые файлы `.wav` через компонент `CustomAudioManager` :PESgn_Noice:.

Поместите аудиофайлы в папку `Audio/`, `Audios/` или `GameResources/`:
...
NML патчит `MusicBox.playSound` и воспроизводит ваши звуки напрямую .
