...
Законы мира - это переключатели в окне **Законы мира**: "старость", "голод", "мирные монстры". Это самая удобная для игроков вещь, которую вы можете добавить, ведь она позволяет включать и выключать функции вашего мода без редактирования конфигурационных файлов.
...
В этом и заключается весь смысл. В любом месте вашего мода:
...
> [!WARNING] Законы мира используют _title, а не просто id
> Почти все остальные ассеты используют свой обычный id как ключ названия. Законы мира требуют `<id>_title`. Ошибетесь - и переключатель появится в игре вовсе без подписи :PESgn_Really:.

> [!TIP] Закон лучше настройки в меню
> Настройки модов живут в меню, которое игрок открывает один раз в жизни. Закон мира находится прямо в игре, по соседству с ванильными законами, сохраняется для каждого мира отдельно и переключается на лету. Если в вашем моде есть переключаемое поведение, ему самое место здесь :wbblessed:.
