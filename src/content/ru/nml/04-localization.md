...
Имя файла **и есть** язык: `en.json`, `cz.json` (упрощённый китайский), `ch.json` (традиционный китайский), `ja.json` (японский), `ru.json` и так далее. Это id, зарегистрированные в `GameLanguageLibrary`, а не угаданные коды ISO. Чешский - это `cs`, а не `cz`. Держите папку мода названной `Locales` с заглавной L; собственные пути ресурсов игры `locales/` - это отдельная вещь.
...
Для значения, нужного только на текущем загруженном языке:
...
`add(string pKey, string pTranslation, bool pReplace = false, string pFileName = "", bool pCheckForCharacters = true)` пишет в текущий текстовый словарь. Существующие ключи остаются без изменений, если только `pReplace` не равен true. Он нормализует ключ через `Underscore()`, так что используйте ключи с подчёркиванием с самого начала. `getText(string pKey, Text text = null, bool pForceEnglish = false)` читает этот словарь; в проверенном исходнике `pForceEnglish` не используется для выбора английского.

> [!NOTE] Текущий текст - это не файл перевода
> Смена языка перестраивает текстовые словари игры. Для переводов, которые должны пережить смену языка, используйте `Locales` или `LM.Add`. Прямой `add` также не обновляет за вас уже существующие текстовые компоненты.

Далее: **[Спрайты и ресурсы](#/nml/sprites-and-resources)** или вывод текста на экран в **[Сообщения и журнал мира](#/nml/messages-and-world-log)**.
