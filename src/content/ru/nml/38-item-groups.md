...
Группа предметов (item) — это категория в окне снаряжения: шлемы, мечи, амулеты. Это точно такой же компактный `BaseCategoryAsset`, который отрисовывает вкладки черт (trait) (см. **[Группы черт и вкладки](#/nml/trait-groups)**), только расположенный в `AssetManager.item_groups`.
...
Игра хранит отдельную корзину предметов для каждой группы и наполняет эти корзины во время инициализации собственных библиотек (library) — то есть **еще до того, как ваш мод начнет выполняться**. У свежесозданной группы корзины попросту нет, и первый же вызов, запрашивающий ее, приведет к ошибке:
...
| `equipment_subtype` | К какому **классу оружия** относится: `sword`, `axe`, `bow`… то, что выбирают культуры (culture) |
...
Группы отрисовываются в порядке `list`, и `add()` ставит вашу последней. Передвиньте её к родственной:
