...
Вы наверняка видели аккуратные зелёные строчки в описании хороших мечей: *«+3 к урону»*, *«огненный»*. Это **модификаторы (modifier) предметов (item)**, и они представляют собой самый быстрый способ сделать экипировку интересной, ведь игра сама случайно распределяет их по выпадающему оружию.

## Простой путь: генератор NML

Модификатор - это `ItemModAsset`, то есть `ItemAsset` в другой шляпе, и живёт он в `AssetManager.items_modifiers`:
...
> [!WARNING] Зарегистрировать недостаточно
> `add()` кладёт ваш модификатор в `list` библиотеки (library), но генератор читает не `list`, а `pools`. Эти пулы заполняются в `linkAssets()`, один раз, при загрузке. Модификатор, который есть только в `list`, существует, у него есть имя, и он никогда ни на что не выпадет :wbfacepalm:.
...
Добавьте `HelloModifiers.Initialize();` в `Main.cs`, и с этого момента игра сможет выдавать его на сгенерированное оружие.
...
| `base_stats` | Прибавка к характеристикам (stats) |
...
Прибавка к характеристикам — это здорово, но модификатор может выполнять и код, и вот тут начинается веселье. Делегат `action_attack_target` срабатывает при каждом успешном ударе:
...
Значение `translation_key` отображается в подсказке предмета, поэтому делайте его коротким — оно занимает одну строчку рядом с характеристиками. Никто не читает абзацы на мече.
