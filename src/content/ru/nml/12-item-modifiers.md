...
> `add()` кладёт ваш модификатор в `list` библиотеки, но генератор читает не `list`, а `pools`. Эти пулы заполняются в `linkAssets()`, один раз, при загрузке. Модификатор, который есть только в `list`, существует, у него есть имя, и он никогда ни на что не выпадет :wbfacepalm:.
