...
`World.world.kingdoms` работает аналогично, см. **[Королевства и фракции](#/nml/kingdoms)**. Вызывайте `isRekt()` для каждого элемента без исключения: эти списки содержат объекты, которые могут умирать прямо в этот момент.
