...
Три компактные библиотеки (library), которые требуются постоянно, как только вы начинаете создавать активные события на карте:
...
| `AssetManager.effects_library` | Чистая визуализация: взрыв, облако (cloud), вспышка света, клуб дыма |
...
| `animated`, `animation_speed`, `frames` | Анимируется ли снаряд (projectile) во время полета |
...
Обе позиции имеют тип `Vector3`. Поле `posV3` клетки (tile) получить проще всего; а `current_position` существа — это `Vector2`, поэтому его нужно предварительно сконвертировать.
...
Заклинание (spell) — это способность, которую существо использует по собственному решению, без участия игрока. ИИ решает, когда её кастовать, на основе `chance`, `cost_mana` и `min_distance`.
...
Поле `action` представляет собой делегат `AttackAction` — ту же сигнатуру, что используют оружейные модификаторы (modifier), поэтому тело заклинания и логика чар оружия взаимозаменяемы.
...
`addSpell()` только добавляет id. Библиотека превращает id в заклинания в `linkAssets()`, при запуске, до вашего мода: пропустите `linkSpells()` у черты (trait) или предмета (item), которые вы зарегистрировали сами, и они ничего не дадут, причём молча.
...
| `cooldown` | Секунды статуса (status) `recovery_combat_action` после, который блокирует любое боевое действие |
| `can_do_action` | Ваше условие, с учётом цели |

> [!WARNING] Выдают их только черты
> Юнит собирает боевые действия из своих черт, а также из подвида (subspecies), клана (clan) и религии (religion), но никогда из снаряжения. Черта хранит id, а игра превратила id в объекты при запуске: вызовите `linkCombatActions()` после `addCombatAction()`, иначе черта будет нести приём, который никто никогда не сделает :PES2_Shrug:.
