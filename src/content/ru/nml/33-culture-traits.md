...
**Культура** (culture) — это совокупность общих привычек и обычаев группы городов. Она определяет, что они строят, что куют, как передают наследство, что читают и что ценят. Черта (trait) культуры — это одна из таких привычек.

Из всех семи систем черт культура обладает наибольшим охватом. Культура распространяется вместе с городами, переживает своего основателя и внедряет свои характеристики (stats) в абсолютно каждого юнита, который к ней принадлежит. Если вам нужен мод, эффект которого плавно разворачивается по всему миру в течение часа игры, эта библиотека (library) создана именно для этого. Большой охват, большая ответственность :PES5_Menace:.
...
Все правила из раздела **[Кастомные черты](#/nml/custom-traits)** действуют и здесь: вызов `add()` перед характеристиками, `path_icon` не генерируется сам, к ID обязательно добавляется префикс. Ниже описано то, что делает черты культуры уникальными. И это самая весёлая часть.
...
Оба вспомогательных метода автоматически устанавливают `is_weapon_trait = true`. Код ремесла обращается к предпочтениям культуры, когда город решает, что ковать; это меняет реальное оружие в руках солдата, а не просто абстрактную цифру. В ванильной игре `bow_lovers` и `spear_lovers` устроены ровно таким образом. Целая культура любителей копий, из двух строк :PESgn_Noice:.
...
| `related_weapons_ids` | Предпочитаемые ID предметов (item). `addWeaponSpecial` добавляет сюда |
...
> [!TIP] Культура или подвид (subspecies)?
