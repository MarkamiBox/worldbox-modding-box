...
| Поле | Назначение |
| --- | --- |
| `rate` | Вес: как часто выбирается по сравнению с другими бедствиями |
| `chance` | Дополнительный бросок шанса после выбора |
| `min_world_population` / `min_world_cities` | Условия, необходимые для возможности срабатывания |
| `type` | `DisasterType.Nature`, `Other`, … |
| `world_log` | ID ассета `WorldLogAsset`: строчка в журнале мира. **Не** текстовый ключ, см. ниже |
| `action` | Ваш код. Это и есть само бедствие |
| `spawn_asset_unit` + `units_min`/`units_max` | Шорткат для «заспавнить N таких существ» |
| `max_existing_units` | Не спавнить новых, если уже существует столько существ |
...
Пишите текст как новостной заголовок, а не как сухое описание. Именно эту фразу игрок увидит в журнале событий мира.
