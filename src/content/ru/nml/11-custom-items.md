...
Первое, что нужно усвоить: **в игре не существует абстрактного предмета (item) «меч» с полем материала, выбираемым на лету**. В игре существуют `sword_wood`, `sword_stone`, `sword_copper`, `sword_bronze`, `sword_silver`, `sword_iron`, `sword_steel`, `sword_mythril`, `sword_adamantine`. Девять совершенно отдельных ассетов, каждый со своей стоимостью, характеристиками (stats) и строкой `material`. То же самое касается каждого доспеха, каждого лука, каждого амулета.
...
| `equipment_subtype` | `sword`, `axe`, `bow`, … Подтип оружия. У культур (culture) есть любимые подтипы |
| `group_id` | Вкладка категории снаряжения. См. **[Группы черт и вкладки](#/nml/trait-groups)** |
| `attack_type` | Ближний или дальний тип поведения (behaviour) |
...
| `minimum_city_storage_resource_1` | Город не начнёт ковать предмет, если запас ресурсов (resource) на складе ниже этого порога |
...
| `projectile` | Для оружия дальнего боя: какой снаряд (projectile) летит. См. **[Снаряды, заклинания и эффекты](#/nml/projectiles-spells)** |
...
| Поле | Что делает |
| --- | --- |
| `action_attack_target` | Срабатывает при каждом успешном ударе |
| `action_special_effect` + `special_effect_interval` | Срабатывает по таймеру, пока предмет экипирован |
| `item_modifier_ids` | Зачарования (modifier), которые могут на нём выпасть. См. **[Зачарования оружия](#/nml/item-modifiers)** |
| `addSpell(id)` + `linkSpells()` | Заклинание (spell), которое может использовать владелец. Связывание вызываете вы сами, см. ниже |
| `addCombatAction(id)` | Компилируется, но на предмете ничего не делает: юнит собирает боевые действия из своих черт (trait) (и подвида (subspecies), клана (clan), религии (religion)), но никогда из снаряжения. Повесьте его на черту, см. **[Снаряды, заклинания и эффекты](#/nml/projectiles-spells)** |

Игра превращает эти id в объекты один раз, при запуске, до загрузки вашего мода. На предмете, который вы зарегистрировали сами, закончите вызовом `linkSpells()` и задайте `decisions_assets` вручную (метода связывания для него нет), иначе выдача ничего не даст. См. **[Свой ИИ](#/nml/custom-ai)**.

## Эффект при удерживании в руках

«Тот, кто держит Тлеющий клинок, становится Быстрым» звучит как черта на предмете. Предметы не несут в себе черт, но они выполняют код по таймеру, пока экипированы (`action_special_effect` из таблицы выше), а **статус** (status) истекает сам по себе. Таким образом, предмет постоянно повторно накладывает короткий статус, и когда предмет исчезает, статус просто прекращается:
...
Молот, которым размахивает строитель, и корзина в руках собирателя — это не предметы экипировки. Это **ручные инструменты (hand tools)**: чисто визуальные спрайты, отображаемые, пока юнит занят определенной задачей (task), и исчезающие по ее завершении.
...
| `colored` | Окрашивание в цвет королевства (kingdom), как у флага |
