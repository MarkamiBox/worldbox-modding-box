...
| `item_modifier_ids` | Зачарования, которые могут на нём выпасть. См. **[Зачарования оружия](#/nml/item-modifiers)** |
| `addSpell(id)` + `linkSpells()` | Заклинание, которое может использовать владелец. Связывание вызываете вы сами, см. ниже |
| `addCombatAction(id)` | Компилируется, но на предмете ничего не делает: юнит собирает боевые действия из своих черт (и подвида, клана, религии), но никогда из снаряжения. Повесьте его на черту, см. **[Снаряды, заклинания и эффекты](#/nml/projectiles-spells)** |
