...
Каждое имя в WorldBox выходит из генератора: юниты, города, королевства (kingdom), кланы (clan), войны (war), книги (book). У ваших существ может быть свой, чтобы деревня тлеющих духов была полна Ашр и Синдоксов, а не заимствовала человеческие имена.
...
| Шаг | Ассет | Что в нём |
| --- | --- | --- |
| Существо | `ActorAsset.name_template_sets` | Список id **наборов имён**. Для каждой культуры (culture) выбирается один |
