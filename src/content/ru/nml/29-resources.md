...
| `give_trait_id`, `give_status_id`, `give_chance` | Черты (trait) и статусы (status), накладываемые при еде |
...
| `supply_give`, `supply_bound_give`, `supply_bound_take` | Поведение (behaviour) при снабжении армий |
...
**Материал** предмета (item) (железо, сталь, мифрил) — это `ItemAsset` в библиотеке материалов, а не `ResourceAsset`, даже если использование материала стоит реальных ресурсов. Это одна из тех областей, где внутренняя терминология игры максимально запутывает. См. **[Кастомные предметы](#/nml/custom-items)**.
...
> Новой *культуре (culture) или ингредиенту* нужен целый производственный цикл: растение, биом (biome) для произрастания, профессия для сбора. Новой *выпечке или блюду* нужны только уже существующие ингредиенты, и рецепт мгновенно встраивается в работающие пекарни и торговые пути. Первое займет неделю, второе — один вечер :PES_ChillPill:.
