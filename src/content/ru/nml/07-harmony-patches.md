...
- **`public static void Postfix(...)`**: это имя **не** ваше. Harmony ищет метод, названный ровно `Prefix`, `Postfix` или `Finalizer`. Напишите `postfix`, и ничего не произойдёт, причём без ошибки :PESgn_ButWhy:.
...
- **Сначала дешёвая проверка.** Первая строка горячего патча должна быть проверкой, после которой можно сделать `return`.
