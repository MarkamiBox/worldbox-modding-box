...
**Черта королевства** — это государственная политика. Не верование и не родословная: указ, принятый короной и действующий на все государство.

Ванильная игра использует эту систему исключительно для одного — налоговых ставок. Это делает ее самой компактной и пустой из всех семи систем черт, а потому и самым заманчивым местом для добавления собственных механик. Никто не конкурирует с вами за это пространство.

| | |
| --- | --- |
| Библиотека | `AssetManager.kingdoms_traits` |
| Класс | `KingdomTrait` |
...
> [!WARNING] Характеристики королевства не доходят до юнитов
> Как и в случае с религией, `kingdom.base_stats` никогда не внедряется в `Actor`. Числа масштаба королевства, видимые в игре, берутся из **личных характеристик монарха** (`king.stats["cities"]` и тому подобных), а не из блока черт государства.
>
...
Поскольку `base_stats` исключен, черта королевства оправдывает свое существование двумя путями.

**Поведенческое решение**, аккуратный и лаконичный вариант:
...
Это общий шаблон для любой политики королевства, не являющейся налоговой ставкой: черта выступает переключателем, а ваш патч реализует поведение. См. **[Патчи Harmony](#/nml/harmony-patches)**.
