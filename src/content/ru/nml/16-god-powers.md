...
- **`name`**: используется поиском интерфейса игры. Оставив его равным id, вы избавите себя от лишней головной боли.
- **`rank = PowerRank.Rank0_free`**: доступно с самого начала, открывать не нужно.
- **`path_icon`**: иконка курсора и инструмента.
- **`unselect_when_window`**: при открытии окна инструмент автоматически деактивируется, чтобы случайно не уничтожить мир под панелью.
- **`click_action`**: ваш код. Получает **тайл, по которому кликнули**, и **id силы**, возвращая `true`, если было выполнено действие.

> [!WARNING] Сигнатура клика: (WorldTile, string)
> `click_action` имеет тип `PowerActionWithID`, поэтому его второй аргумент - это **id силы в виде string**, а не `GodPower`. Существует и второе поле, `click_power_action`, принимающее `(WorldTile, GodPower)`. Ошибка в сигнатуре приведет к ошибке компиляции с совершенно невнятным текстом :PES_DaFuq:.
