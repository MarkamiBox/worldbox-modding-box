...
Эпоха Углей каждые восемь секунд вызывает дождь из углей, затемняет экран пеплом, ускоряет распространение огня вдвое и делает города чуть более лояльными. Новый мир может разместить ее в слоте 4 своего колеса, а случайный выбор колеса может поставить ее в любую позицию.

> [!WARNING] Три вещи, которые библиотека сделала при запуске
...
| `clouds` + `cloud_interval` | Создаваемые облака и интервал их появления |
