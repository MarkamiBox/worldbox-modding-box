...
Вы зарегистрировали божественную силу. Никто не может нажать на нее, ведь `GodPower` - это просто данные: то, на что игрок физически нажимает, называется **PowerButton**, а кнопки живут внутри **вкладок** на нижней панели.

## Создание собственной вкладки

Вы *можете* добавить кнопку в одну из стандартных ванильных вкладок. Не делайте этого. Они и так переполнены, игра сортирует дочерние элементы по имени, а сама панель прокручивается, так что ваша кнопка уедет в такое место, куда игрок никогда в жизни не доскроллит :PESgn_ToughLuck:.

Одна собственная вкладка - и всё, что вы добавляете, будет аккуратно собрано вместе и легкодоступно:
...
> `PowersTab` читает своего родителя в `Start()` Unity, который на объекте, только что выданном `CreateTab`, ещё не выполнился. Вызовите там `recalc()` — и весь этап падает с `NullReferenceException` в `PowersTab.setNewWidth()`, сила не регистрируется, а вкладка так и не появляется :wbfacepalm:.
>
> ```csharp
> public void Update()
> {
...
`tab.recalc()` как раз и выполняет разметку кнопок. Забудете его - и вкладка будет казаться пустой, хотя сами кнопки на ней физически присутствуют.
...
Используйте `CreateGodPowerButton` для всего, что игрок нацеливает на карте (спавн, удар, осмотр юнита, размещение здания), и `CreateSimpleButton` для глобальных действий (открыть окно, переключить режим).
