...
**Религия** (religion) принадлежит городам и королевствам (kingdom), распространяется через обращение в веру, пишет священные книги (book) и может вершить **обряды**: меняющие мир заговоры (plot), которые ее последователи пытаются воплотить самостоятельно. Черта (trait) религии — это одно верование.

| | |
| --- | --- |
| Библиотека (library) | `AssetManager.religion_traits` |
| Класс | `ReligionTrait` |
...
> [!WARNING] Характеристики (stats) религии не передаются юнитам
> Это единственная система черт, чьи `base_stats` никогда не попадают в `Actor`. `Actor.updateStats()` объединяет подвиды (subspecies), кланы (clan), языки и культуры (culture). **Религии в этом списке нет.**
>
> Поэтому черта религии преображает мир через то, что она *делает* (обряд, трансформация биома (biome), хук действия), а не через числовые характеристики. Назначить ей `base_stats["damage"] = 10` — это холостой выстрел в пустоту и самый частый способ бессмысленно потратить день :PES4_BigSad:.
...
> Новые религии выбирают начальные черты из пула, который `BaseTraitLibrary.linkAssets()` формирует во время загрузки игры, до появления вашего мода. Установка флага на вашей черте сама по себе ничего не меняет: черта никогда не попадет в этот пул и не появится случайно у основателя. Добавьте её сами с весом, используемым в ванильной игре:
>
> ```csharp
...
Черта религии с заполненным полем `plot_id` становится **обрядом**. Религия собирает свои обряды в коллекцию `possible_rites`, и правители со жрецами пытаются исполнить их самостоятельно, как только выполнены условия заговора. Вы пишете веру, жрецы делают остальное :wbpray:.
...
Черта религии также может наделять заклинанием (spell) или поведенческим решением (decision), что зачастую оказывается куда органичнее простого таймера:
