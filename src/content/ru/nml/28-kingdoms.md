...
Каждое существо в WorldBox принадлежит к определенному королевству (kingdom). Не только цивилизованные народы: волки принадлежат к волчьему королевству, бандиты — к фракции бандитов, а нейтральная курица — к нейтральному королевству. Класс `KingdomAsset` — это **тип** фракции, а не конкретное государство на карте.
...
Как и у существ, у королевств есть идентификаторы вида `$TEMPLATE$` именно для этой задачи (task):
...
Без этого шага ваше существо появится в том королевстве, которое использовал донор клона (обычно в человеческом), породив немало путаницы :PES5_Hmmmm:.
...
| Поле | Что делает |
| --- | --- |
| `civ` | Строит города, ведет войны (war), имеет правителя |
...
Ассет королевства сам по себе — лишь ярлык. Очень официальный ярлык, но всё же ярлык. Вот остальные библиотеки (library), с которыми взаимодействует полноценная фракция:
...
| Черты (trait) королевства | `AssetManager.kingdoms_traits` | Политика, преимущественно налоги. См. **[Черты королевства](#/nml/kingdom-traits)** |
| Профессии королевства | `AssetManager.job_kingdom` | Над чем работает ИИ фракции |
| Задачи королевства | `AssetManager.tasks_kingdom` | Дерево поведения (behaviour) за этими работами |
...
Три небольшие библиотеки формируют политический климат мира, и все три представляют собой списки расчетных функций. Никаких чувств, только числа:
...
Королевства — не единственные сущности со знаменами: культуры (culture), религии (religion), кланы (clan), языки, подвиды (subspecies) и семьи имеют собственные библиотеки элементов (`AssetManager.culture_banners_library` и др.). У каждой есть один `main` ассет со списками путей к спрайтам, из которых новая культура случайно выбирает индекс.
...
Текстуры подгружаются по одной при отрисовке знамени, поэтому сбрасывать кэш не нужно. Индекс за пределами списка откатывается к 0, благодаря чему сохранения с вашим модом открываются и без него. Сверяйте размеры спрайтов в **[UnityExplorer](#/toolbox/unity-explorer)** перед рисованием. Угадаете размер наугад и получите флаг больше города, над которым он висит :wbfacepalm:.
