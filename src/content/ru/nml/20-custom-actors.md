...
> `AssetManager.<library>.clone(newId, sourceId)` внутри вызывает `add()`. Так работает каждая библиотека. Вызвать потом `add()` самому - это двойная регистрация: библиотека удаляет первую копию, пишет ошибку и добавляет её заново. Безвредно, но это шум в логе, из-за которого настоящие ошибки труднее найти, и первое, что заметит проверяющий.
>
> Обратная сторона - хорошая новость: **после клонирования `base_stats` уже существует**, так что правило «характеристики после add» из **[Своих черт](#/nml/custom-traits)** уже выполнено.
