...
Новое существо - это тот самый мод, который хотят создать абсолютно все, но почти никто не доводит до конца. Дело в том, что `ActorAsset` несет в себе анимации, текстуры, звуки, таксономию, диету, флаги ИИ, геном, культуру и параметры. Ошибетесь хотя бы в одном из них - и получите невидимое существо, бездвижно стоящее посреди океана.
...
| `$civ_advanced_unit$` | Полноценная цивилизация: города, государства, культура, религия. То, на чем построены люди, эльфы, орки и гномы |
...
```
> [!WARNING] Загрузите тень сами, иначе игра ругается на каждого актёра
> `ActorAssetLibrary` на старте проходит по своему списку и вызывает `loadShadow()` у каждого актёра: тот грузит спрайт из `shadows/<shadow_texture>` и замеряет его. Это случилось до того, как ваш мод что-либо зарегистрировал, так что тень вашего актёра остаётся `(0.00, 0.00)`, и игра пишет по ней ошибку ассета три раза: взрослый, яйцо и детёныш :wbfacepalm:.
>
> `loadShadow()` — `internal`, так что нужна **publicized** `Assembly-CSharp.dll`, как и для остального в этом гайде. Если её нет, поставьте `asset.shadow = false;`: без тени, зато без ошибки.


> [!WARNING] clone() уже выполняет регистрацию
> Метод `AssetManager.<library>.clone(newId, sourceId)` сам вызывает `add()` под капотом. Так работают абсолютно все библиотеки ассетов. Ручной вызов `add()` после клонирования приведет к дублированию: библиотека удалит первую копию, выдаст ошибку в лог и добавит ассет заново. Это не сломает игру, но создаст лишний шум в логах и сразу же вызовет вопросы у любого ревьюера.
>
> С другой стороны, есть и отличная новость: **после клонирования `base_stats` уже инициализирован**, а значит, строгое правило "статы только после add" из руководства по **[Кастомным чертам](#/nml/custom-traits)** уже выполнено.
...
| Поле | Что делает |
| --- | --- |
| `civ` | Существо цивилизации: города, королевства, профессии, войны. `false` = дикое животное |
...
| `can_have_subspecies` | Мутируют ли они в подвиды через несколько поколений |
| `has_ai_system` | Запускает ли существо базовую систему поведения |
...
| `can_attack_buildings` | Способны ли атаковать и ломать здания |
| `has_soul`, `can_receive_traits`, `can_be_cloned` | Что божественные силы могут с ними делать |
...
Метод `spawnNewUnit` публичен и принимает опциональные аргументы: звук спавна, спецэффект чуда, высоту появления, конкретный подвид и выдачу стартовых предметов.

Привяжите к нему кнопку божественной силы - и получите готовый спавнер. См. **[Вкладки сил и кнопки](#/nml/power-buttons)**.

## Подвиды

Подвиды (Subspecies) - это вариации, в которые вид эволюционирует и мутирует со сменой поколений. У них своя собственная библиотека черт, отделенная от черт существ, и свой список групп:
...
Всё, что описано выше, - это просто страничка кода. Настоящий колоссальный труд - это визуальная часть: существу требуется полный комплект анимаций в правильном атласе, нужного размера и с точнейшими точками привязки (pivot). Есть два честных пути:
