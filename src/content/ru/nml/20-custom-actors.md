...
Лодка - это тоже существо (actor), только с `is_boat = true`, а шаблоны - это `$boat_trading$` и `$boat_transport$`. Клонируйте готовую, `boat_trading_human` или `boat_transport_human`, и получите характеристики, пушку и ИИ лодки бесплатно. Цивилизация выбирает свои лодки не по виду: их называет её **архитектура**, в полях `actor_asset_id_trading`, `actor_asset_id_transport` и `actor_asset_id_boat_fishing`.

Что не достаётся бесплатно - это графика. Изображение лодки берётся не из `texture_asset`: игра загружает его из `actors/boats/<id лодки>/`, по имени спрайта, при первой же отрисовке:

```text Mods/HelloBox/
HelloBox/
└── GameResources/
    └── actors/boats/hello_boat_trading/
        ├── normal.png      # пока поднята или в магните, и запасной вариант
        ├── broken.png      # обломки
        ├── 0@0.png         # плавание, по паре на каждое направление:
        ├── 0@1.png         # <угол>@0 и <угол>@1
        └── ...             # для 0, 45, 90, 135, 180, -45, -90, -135
```

`normal` и `broken` обязательны: без них загрузчик бросит `KeyNotFoundException`. На воде игра подставит ближайший угол, который вы нарисовали, но аватар юнита в инспекторе требует все восемь, так что рисуйте все восемь.

`ActorAnimationLoader.loadAnimationBoat("hello_boat_trading")` публичный, и именно его вызывает игра. Вам он не обязателен, но вызов его один раз сразу после клонирования превращает отсутствующий спрайт в ошибку при загрузке, в вашем собственном коде, а не посреди морской войны :wbsmirk:.

> [!NOTE] На карте нет метки вашей лодки
> Маленькие иконки лодок на уменьшенной карте берутся из `AssetManager.actor_library.list_only_boat_assets`, списка, который библиотека строит один раз при загрузке. Добавьте туда свою лодку, если хотите метку.
