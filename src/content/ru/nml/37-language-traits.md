...
**Язык** принадлежит городам и королевствам (kingdom), претерпевает изменения по мере распространения и выступает носителем, на котором пишутся **книги** (book). Черта (trait) языка — это свойство самого устного и письменного слова.

Это самая компактная из всех семи систем черт, обладающая самым узкоспециализированным хуком: кодом, срабатывающим в момент, когда кто-то **читает книгу** на данном языке. Да, правда :wbscroll:.

| | |
| --- | --- |
| Библиотека (library) | `AssetManager.language_traits` |
| Класс | `LanguageTrait` |
...
> Книга на вашем языке — это неторопливый, гармонично вписанный в мир способ наделить жителя чертой или статусом (status). Она путешествует по библиотекам, охватывает поколения, и игрок наблюдает за этим процессом. Практически никто не делает моды на эту механику, поэтому заняться ей вдвойне интересно :PES4_Classy:.

## Новые языки, случайно получающие черту при создании

Помимо ручной выдачи, черта языка может установить флаг `spawn_random_trait_allowed`, чтобы выбираться при создании нового языка — точно так же, как культура (culture) выбирает свои начальные черты. Та же ловушка, что и на всех остальных страницах о чертах:

> [!WARNING] `spawn_random_trait_allowed` читается только один раз, при запуске
> Новые языки выбирают начальные черты из пула, который `BaseTraitLibrary.linkAssets()` формирует во время загрузки игры, до появления вашего мода. Установка флага на вашей черте сама по себе ничего не меняет: черта никогда не попадет в этот пул и не появится случайно у нового языка. Добавьте её сами с весом, используемым в ванильной игре:
>
> ```csharp
